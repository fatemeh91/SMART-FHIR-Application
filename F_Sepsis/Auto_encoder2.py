...
#    @define_scope
#    def Optimizer(self):  
#        final_loss = tf.add(tf.reduce_mean(tf.pow(tf.subtract(self.X, self.decoder), 2)),self.Accumulative_loss)
#        if self.opt == 'Adam':
#            optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=self.beta1,beta2=self.beta2,epsilon=self.epsilon,use_locking=False).minimize(final_loss)
#        elif self.opt=='RMS':    
#            optimizer = tf.train.RMSPropOptimizer(self.learning_rate).minimize(final_loss)
