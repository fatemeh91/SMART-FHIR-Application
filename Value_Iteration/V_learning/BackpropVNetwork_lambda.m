function [Loss, dL_WL, dL_St] = BackpropVNetwork_lambda(S_t, r_t, VStp1, WL)
% INPUT:
...
Loss=0;  backup_length=3;  % forward approach
if nargout >1
    dL_WL=cell(num_layers,1);  dL_St = zeros(size(S_t));
    for la=1:num_layers, dL_WL{la}=zeros(size(WL{la})); end
    for t=1:Ttot
        backup_indx = t:min(t+backup_length-1,Ttot); gam = gamma.^(0:length(backup_indx));
        % Immediate reward + discounted long-term reward of choosing the best action in state S_t+1
        y = gam(1:end-1)*r_t(backup_indx(1:end)) + gam(end) * VStp1(backup_indx(end),:)*double((backup_indx(end)~=Ttot)); % this is a scaler
